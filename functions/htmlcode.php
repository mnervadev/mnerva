...
function htmlcode($orimessage) {
	...
	// hashtag link
	$message = preg_replace($regex, '<a href="hashtags.php?tag=$1" class="hshCl">$0</a>', $message);
	// mention link
    $message = preg_replace($regex_mention, '<a href="$1">$0</a>', $message);
...
function htmlcodecomment($orimessage) {
 ...
 function hashtag($orimessage){
  //filter the hastag
  preg_match_all('/(^|[^a-zA-ZÇŞĞÜÖİçşğüöı0-9-_])#([a-zA-ZÇŞĞÜÖİçşğüöı0-9-_]+)/i', $orimessage, $matched_hashtag);
  $hashtag = '';
  //if hashtag found
  if(!empty($matched_hashtag[0])){
   ...
   foreach ($matched_hashtag[0] as $matched) {
    ...
    $hashtag .= preg_replace('/[^a-zA-ZÇŞĞÜÖİçşğüöı0-9-_]+/i', '', $matched).',';
...
function fullnm($fullName) {
	...
	$regex_mention = "/@+([a-zA-Z0-9-_]+)/";
	// hashtag link
	$fullName = preg_replace($regex, '<a href="hashtags.php?tag=$1" class="hshCl">$0</a>', $fullName);
	// mention link
